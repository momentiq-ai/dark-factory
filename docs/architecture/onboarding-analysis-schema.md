...
| `ci.workflows[].firstRunCommand` | `string \| null` | — | (raw YAML scan) | First TRUE single-line `run:` command in this workflow's raw YAML (>10 chars, not a block scalar marker like `\|` / `>`). `null` when the workflow has only multi-line `run: \|` blocks or `uses:`-only steps. Captured from raw text (not the parsed YAML AST) so the seeder can embed a candidate the metric-4 validator's `^\s*-?\s*run:` regex will recognize — capturing via the AST would silently lose the single-vs-multiline distinction. Added in fix #138 to support the runbook seeder's donor fallback (when no deploy-named workflow has a single-line `run:`, the seeder appends a non-deploy-named workflow with `firstRunCommand` so a runbook embeds a verbatim CI line). |
