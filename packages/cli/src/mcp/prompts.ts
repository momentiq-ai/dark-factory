...
export function registerPrompts(
  ...
  server.registerPrompt(
    ...
    ({ issue }) => {
      // Single-sourced from rehydrate-contract.ts so the prompt and the
      // df_rehydrate / df_accept tool descriptions can never drift.
      const template = rehydrationRitual({ issue });
      return {
